...
class List extends Component {
    ...
    render() {
        ...
        titles.unshift(<th className='sl' key="#">{cms.sl}</th>);
        if (select) titles.unshift(<th key="select_all">
            <input type="checkbox" onClick={selectHandler} className="select_all" />
        </th>);

        const content = array.map((item, index) => {
            if (limit && index >= limit) return null;
            let inside = [<th className='sl' key={'primary' + index}>{(show === 'All' ? 0 : (page - 1) * show) + index + 1}</th>];
            if (select) inside.unshift(<th key={'secondary' + index}>
                <input type="checkbox" value={item._id} />
            </th>);
            fields.forEach(({ key, minWidth, fixed, className = '' }) => {
                inside.push(<td className={className} style={updateObject({ minWidth, maxWidth: 250 }, fixed ? { position: 'sticky', right: 0 } : {})} key={key}>{item[key]}</td>);
            });
        ...
        return (
            <div className={'UI List shadow ' + outerClassName}>
                ...
                <div className="body">
                    <div className={`table-responsive scrollbar-${window.APP_PRIMARY_COLOR}`}>
                        <Table dark={dark} bordered={bordered} hover borderless={borderless}>
                            <thead className='title'><tr>{titles}</tr></thead>
