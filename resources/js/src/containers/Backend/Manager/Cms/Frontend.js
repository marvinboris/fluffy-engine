...
class Frontend extends Component {
    ...
    render() {
        const {
            content: {
                cms: {
                    pages: { backend: { components: { form: { save } }, pages: { cms: { icon, title, frontend } } } }
                }
            },
        ...
        const mainContent = languages.map(language => {
            ...
            return <div key={Math.random()} className={language.abbr === abbr ? "" : "d-none"}>
                <LanguageWithPages cmsExample={cmsExample.pages[process.env.MIX_DEFAULT_LANG].frontend} cmsValue={cmsValue} language={language} part='frontend' />
            </div>
        });
        ...
        return <div className='Cms Frontend'>
            ...
            <Loading loading={this.state.isMounted && loading} isAuthenticated>
                <div className='content'>
                    {errors}
                    <Form onSubmit={this.submitHandler} icon={icon} title={frontend} link={`/${role}/cms`} innerClassName="row">
                        {content}
                    </Form>
                </div>
            </Loading>
